...
if( instance_exists(objScLogConsole) )
{
    ...
    if( instance_number(objScLogConsole) > 1 ){
        ...
        draw_set_font(logInstance.logFont);
        ...
        if( logLineWidth > scWindowTreshold ){
            ...
            if( lastSpace != -1 ){
                ...
                ScWriteToLog(SCLOG_OVERFLOW, string_copy(message, lastSpace + 1, string_length(message) - lastSpace));
            }else{
                // No spaces found just cut it.
                logInstance.lines[logInstance.maxLines - 1] = string_copy(message, 1, maxCharsInWidth);
                ScWriteToLog(SCLOG_OVERFLOW, string_copy(message, maxCharsInWidth + 1, string_length(message) - maxCharsInWidth));
            }
        }
        else{
